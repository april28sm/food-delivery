...
from PolicyHandler import wheneverRejected_Cancel
...
from PolicyHandler import wheneverRejected_CancelPayment
...
def consume(msg):
    ...
    if data['eventType'] == "Rejected":
        wheneverRejected_Cancel(data)
    if data['eventType'] == "Rejected""Accepted":
        wheneverRejected_UpdateStatus(data)
        wheneverAccepted_UpdateStatus(data)
    if data['eventType'] == "OrderCanceled""Rejected":
        wheneverOrderCanceled_CancelPayment(data)
        wheneverRejected_CancelPayment(data)
